    ...
    *,
    ...
    try:
        ...
        for idx, pair in enumerate(task_data.get("train", [])):
            print_grid(
                pair.get("input"),
                f"Train {task_id} input {idx}",
                background_color=bg_color,
            )
            print_grid(
                pair.get("output"),
                f"Train {task_id} output {idx}",
                background_color=bg_color,
            )
...
def run_test_evaluation(
    ...
    *,
    ...
    for t_idx, pair in enumerate(pairs):
        ...
        pair_data = {"train": [{"input": pair["input"], "output": pair["output"]}]}
        bg_color = determine_background_color(
            pair_data, pixel_threshold_pct=bg_threshold, debug=False
        )
        print_grid(pair["input"], f"Test {t_idx} input", background_color=bg_color)
        print_grid(
            pair["output"], f"Test {t_idx} expected", background_color=bg_color
        )
